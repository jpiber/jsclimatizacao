...
## Branding / favicon
"JS" monogram favicon in the brand palette (navy #0B132B frame + cyan #38BDF8→#0284C7
gradient plate, ink #041525 letters). `frontend/public/favicon.svg` is hand-written vector;
`favicon.ico` (16/32/48), `favicon-32x32.png`, `favicon-16x16.png` (borderless for legibility)
and `apple-touch-icon.png` (180) are generated by `python /app/scripts/make_favicon.py`
(Pillow; Liberation Sans Bold — no rsvg/imagemagick/cairosvg in the pod). Re-run that script
to regenerate after editing the design.
